...
# backend Dockerfile (2 cases on different lines, so no need for the "g" flag)
sed \
...
# wait-for-db.sh (healthcheck workaround for podman v4.x, kept as a redundant HC for podman v5 stack)
sed \
  -e "s|%%DB_HOST%%|${MINIAPP_DB_HOST}|" \
...
sed \
    -e "s|%%HTTPS_PORT%%|${MINIAPP_TRAEFIK_HTTPS_PORT_HOST}|" \
    traefik/templates/dynamic.yml.template > traefik/dynamic.yml
