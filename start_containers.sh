...
# UPDATED: 20AUG2025
# OWNER  : XCS HornetGit
# TODO: service tag consistency and correspondance between the env, compose and this script to be better adjusted.
# NOTES:
...
# drawback: this way implies that the HEALTHCHECK are set in the Dockerfile, and not in the compose file.
...
build_and_run_oneliner() {
    ...
    for service in "${service_order[@]}"; do
        ...
        if [[ -n "${result_services[$service]}" ]] && [[ " ${services_list[*]} " =~ " ${service} " ]]; then
            ...
            set_command "podman rm -f \"$container_name\"" 2>/dev/null || true
...
main() {
    ...
    execute_build_and_run # to be debugged 
    run_command
